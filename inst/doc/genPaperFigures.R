...
myTheme <- theme_bw()+theme(title=element_text(size=32), text=element_text(size=24), 
    axis.title.y=element_text(vjust=1.5), panel.grid=element_blank())
...
df <- melt(df, id="t")
ggplot(df, aes(x=t, y=value, color=variable, linetype=variable))+geom_line(size=1)+
...
df <- data.frame(halfLife=halfLifeSeq, rssT2=ratioSteadyState(T1, T2, halfLifeSeq), 
...
ggplot(df, aes(x=halfLife, y=value, color=FP2))+
    ...
    geom_segment(aes(x=0, xend=TA, y=ratioSteadyState(T1, T3, TA), 
        yend=ratioSteadyState(T1, T3, TA)), linetype="dashed", colour="darkgrey")+
    geom_segment(aes(x=0, xend=TB, y=ratioSteadyState(T1, T3, TB), 
        yend=ratioSteadyState(T1, T3, TB)), linetype="dashed", colour="darkgrey")+
...
tRangeFP1 <- c(1, 50)
tRangeFP2 <- c(10, 500)
colRamp <- colorRampPalette(colors=c('darkgreen','yellow', 'orange', 'red'))(100)
...
res <- lapply(pSeq, function(p) lapply(T2, function(T) 
    simulatedSignalN(T1, T2=T, TA, TB, sigmaAdd, nRealizations, p=p, E=0)))
dfFP2 <- lapply(seq_along(res), function(i) {
    df2 <- lapply(seq_along(res[[i]]), function(j)
        return(data.frame(p=pSeq[i], T=T2[j], D=res[[i]][[j]])))
    df2 <- do.call("rbind", df2)
    return(df2)
})
dfFP2 <- do.call("rbind", dfFP2)
dfFP2$FP <- "FP2"
dfFP2s <- dfFP2 %>% group_by(p, T, FP) %>% 
    summarise(D.mean=mean(D, na.rm=TRUE), D.sd=sd(D, na.rm=TRUE))
dfFP2s$D0 <- simulatedSignal(T1, dfFP2s$T, TA, TB, sigmaAdd=0, p=dfFP2s$p)
T1 <- getSpacedSeq(c(1, 30), n=100)
T2 <- 100
res <- lapply(pSeq, function(p) lapply(T1, function(T) 
    simulatedSignalN(T1=T, T2, TA, TB, sigmaAdd, nRealizations, p=p, E=0)))
dfFP1 <- lapply(seq_along(res), function(i) {
    df2 <- lapply(seq_along(res[[i]]), function(j)
        return(data.frame(p=pSeq[i], T=T1[j], D=res[[i]][[j]])))
    df2 <- do.call("rbind", df2)
    return(df2)
})
dfFP1 <- do.call("rbind", dfFP1)
dfFP1$FP <- "FP1"
dfFP1s <- dfFP1 %>% group_by(p, T, FP) %>% 
    summarise(D.mean=mean(D, na.rm=TRUE), D.sd=sd(D, na.rm=TRUE))
dfFP1s$D0 <- simulatedSignal(T1=dfFP1s$T, T2, TA, TB, sigmaAdd=0, p=dfFP1s$p)
df <- bind_rows(dfFP1s, dfFP2s)
df$p <- paste("p =", df$p, "molecules/minute")
df$FPline <- df$FP
df$FPline <- paste0(df$FP, "line")
ggplot(df, aes(T, D.mean, colour=FP))+geom_point()+myTheme+
    xlab("FP maturation time (minutes)")+ylab(expression(S))+
    scale_color_manual(values=c("FP1"="darkgreen", "FP2"="red", "FP1line"="black", "FP2line"="black"))+
    scale_x_log10(breaks=getBreaks10(range(df$T)))+
    geom_errorbar(aes(ymin=D.mean-D.sd, ymax=D.mean+D.sd))+
    geom_line(aes(T, D0, colour=FPline), data=df)+
...
ggplot(dfs, aes(T, D.sd/D.mean, color=p))+geom_point()+myTheme+
    scale_x_log10(breaks=getBreaks10(range(dfs$T)))+
    scale_y_log10(breaks=getBreaks10(range(dfs$CV)))+
    xlab("FP2 maturation time (minutes)")+ylab(expression(sigma[S]~"/"~mu[S]))+
    geom_line(aes(T, CV), data=dfFit)+
...
AnormSummarizeSamples <- apply(AnormSummarizeViews, c(1, 2, 4), mean, na.rm=TRUE)
...
F <- FRETdata
myColReduced <- myCol[names(myCol) %in% F$tFT]
F$tFT <- paste0(F$tFT, "-sfGFP")
F$tFT <- as.factor(F$tFT)
o <- match(levels(F$tFT), paste0(names(myCol), "-sfGFP"))
F$tFT <- factor(F$tFT, levels=levels(F$tFT)[o])
ggplot(F, aes(tFT, FRET, fill="white"))+geom_boxplot(outlier.size=0, fill="white")+
...
df <- data.frame(
    ...
    tdTomFret=signal(T1=T1, T2=FPestimate["tdTom"], TA=Tfront, TB=Tseq, E=Fs["tdTom-sfGFP"]),
    TagRFPNoFret=signal(T1=T1, T2=FPestimate["TagRFP"], TA=Tfront, TB=Tseq, E=0),
    TagRFPFret=signal(T1=T1, T2=FPestimate["TagRFP"], TA=Tfront, TB=Tseq, E=Fs["TagRFP-sfGFP"]))
df <- melt(df, id="halfLife")
...
ggplot(df, aes(x=halfLife, y=value, color=tFT, linetype=FRET))+
    geom_line(size=1)+
    scale_color_manual(values=as.vector(myColReduced), guide=guide_legend(title="timer"))+
    ...
    theme(legend.position=c(0.77, 0.85), legend.key.width=unit(2, "cm"), legend.box.just="left")
...
rFun <- function(type, r0, tf, slopePrefactor=0.0015, stepPositionPrefactor=0.1) {
...
dfk <- melt(dfk, id="t")
gk <- ggplot(dfk, aes(t, value))+geom_line(size=1)+facet_wrap(~variable, ncol=1)+
    xlab("time (minutes)")+ylab(expression(paste("degradation rate (minutes"^{-1}, ")")))+
    scale_x_continuous(breaks=myBreaks)+myTheme
