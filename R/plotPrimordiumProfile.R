plotPrimordiumProfile <- function(x, add=FALSE, ylab="ratio", lwd=2, cex.lab=1.5, cex.axis, xlim=NA, ylim=NA, main="", col="black", lty=1, alpha=0.3) {
    ...
    if(!add) {
        ...
        plot(-xPts, yMedian, xlab=xlab, ylab=ylab, type="l", lwd=lwd, 
            axes="F", cex.lab=cex.lab, xlim=xlim, ylim=ylim, main=main, col=col, lty=lty)
    ...
    polygon(xPoly, yPoly, col=rgb(red=0, green=0, blue=0.5, alpha=alpha), border=NA)
}
