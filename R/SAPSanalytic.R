...
solveForHeatPlot <- function(mTimeMin, mTimeMax, pAgeMin, pAgeMax,
    ...
    if (channel == 2) ratioSolve=sapply(gridSolveList,
        function(x) ratioSteadyState(T1=Tfixed, T2=log(2)/x[1], halfLife=log(2)/x[2], E=E, f=f))
    else if (channel == 1) ratioSolve <- sapply(gridSolveList,
        function(x) ratioSteadyState(T2=Tfixed, T1=log(2)/x[1], halfLife=log(2)/x[2], E=E, f=f))
...
genRatioHeatmap <- function(tRangeFP, Tfixed, TA, TB, channel=2, E=0, f=1, n=110, ramp=rainbow(100)) {
    x <- solveForHeatPlot(mTimeMin=tRangeFP[1], mTimeMax=tRangeFP[2], pAgeMin=TA, 
        pAgeMax=TB, channel=channel, Tfixed=Tfixed, E=E, f=f, seqLength=n)
...
genTimeSteadyStateHeatmap <- function(tRangeFP2, tRangeHlife, n=110, ramp=rainbow(100)) {
    ...
    hmap <- qplot(x=grid$mTime, y=grid$hLife, colour=grid$tss, shape=I(15),
        xlab="FP2 maturation time (minutes)", ylab="protein half-life (minutes)")+
...
ratioTimeDependent <- function(T1, T2, halfLife, t, E=0, f=1) {
    ...
    m1 <- log(2)/T1
    m2 <- log(2)/T2
    ratio <- (exp(t*m1) * (-1+exp(k*t))*f*(k+m1)*(k+exp(t*m2) * (-k+(-1+exp(k*t))*m2)))/((-k+exp(t*m1) * (k-(-1+exp(k*t))*m1)) * (E*k+exp(t*m2) * (-(-1+E+exp(k*t)) * k+(-1+E) * (-1+exp(k*t))*m2)))
    return(ratio)
}
...
x1 <- function (p, m, k, t, f=1) 
    f*p/(k * (k + m)) * exp(-(k + m) * t) * (k - exp(m * t) * (k - 
        m * exp(k * t) + m))

x1ss <- function(p, m, k, f=1) f*p*m/(k*(k+m))

x1fretFP1 <- function(p, m1, m2, k, t, E=0, f=1)
    (exp(-t*(k+m1+m2))*f*p*(k+exp(t*m1)*(-k+(-1+exp(k*t))*m1))*((exp(t*m2)*(-1+exp(k*t))+E*(-1+exp(t*m2)))*k-(-1+E)*exp(t*m2)*(-1+exp(k*t))*m2))/((-1+exp(k*t))*k*(k+m1)*(k+m2))
...
inflectionSlope <- function(p, m, k, a) -(((-1+a)*m*((k+m)/k)^(-(k/m))*p)/(k+m))

inflectionOffset <- function(p, m, k, a)
    ((k+m)/k)^(1-k/m)*(m*(k+m)*((k+m)/k)^(k/m)*p+(-1+a)*p*(m*(2*k+m)+k*(k+m)*log((k+m)/k)))/(k+m)^3
